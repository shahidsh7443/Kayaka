<?php
	...
	function print_portfolio($item_xml){
		...
		print_item_header( $header, $additional );
		...
		if($portfolio_type == 'Portfolio'){
			...
			print_filter_portfolio( $port_size, $show_title, $show_tag, $category, $header);
		}else if($portfolio_type == 'jQuery Filter Portfolio'){
			print_jquery_filter_portfolio( $port_size, $show_title, $show_tag, $category, $header);
	...
	function print_filter_portfolio( $port_size = "1/4", $show_title = true, $show_tag = true, $parent_category, $header){
		global $port_div_size_num_class, $sidebar_type;
		...
		echo '<div class="portfolio-filer-left-bar"></div>';
		echo '<div class="portfolio-filer-left-bar"></div>';	
		...
		foreach($category_lists as $category){
			if( $category == 'All'){
				...
				echo '<li><span> / </span><a href="' . esc_html(add_query_arg(array('filter'=>$category))) . '" class="' . $active . '">'; 
		...
		echo "</ul>";
		echo '<div class="portfolio-filter-header-divider"></div>';
		echo '<div class="clear"></div>';
		echo '</div>';
	...
	function print_jquery_filter_portfolio( $port_size = "1/4", $show_title = true, $show_tag = true, $parent_category, $header){
		global $port_div_size_num_class, $sidebar_type;
		global $gdl_admin_translator;	
		if( $gdl_admin_translator == 'enable' ){
			$translator_all = get_option(THEME_SHORT_NAME.'_translator_all', 'All');
	...
	function print_page_item($item_xml){
		...
		while( have_posts() ){
			...
			if( !empty($thumbnail_id) ){
				...
				echo '<span class="hover-icon hover-link"></span>';
				echo '</span>';
				echo '</a>';
				echo '<img src="' . $thumbnail[0] .'" alt="'. $alt_text .'"/>';
