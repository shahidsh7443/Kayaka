<?php
	...
	*/
	...
	function get_youtube($url, $width = 640, $height = 360, $type = 'youtube', $return = false){
		...
		if( !$return ){
			echo '<iframe src="https://www.youtube.com/embed/' . $id[1] . '?wmode=transparent' . $attr . '" width="' . $width . '" height="' . $height . '" ></iframe>';
		}else{
			return '<iframe src="https://www.youtube.com/embed/' . $id[1] . '?wmode=transparent' . $attr . '" width="' . $width . '" height="' . $height . '" ></iframe>';
		}
	}
	...
	function get_vimeo($url, $width = 640, $height = 360, $return = false){
		...
		if( !$return ){
			echo '<iframe src="https://player.vimeo.com/video/' . $id[1] . '?title=0&amp;byline=0&amp;portrait=0" width="' . $width . '" height="' . $height . '"></iframe>';
		}else{
	...
	function gdl_submit_contact_form(){
		...
		if(empty($_POST)){
			$return_data['value'] = __('Cannot send email to destination. No parameter receive form AJAX call.', 'gdl_front_end');	
			die ( json_encode($return_data) );
		}
		
		$name = $_POST['name'];		
		if(empty($name)){
			$return_data['value'] = __('Please enter your name.', 'gdl_front_end');
			die ( json_encode($return_data) );
		}
		
		$email = $_POST['email'];
		if(empty($email)){
			$return_data['value'] = __('Please enter a valid email address.', 'gdl_front_end');
			die ( json_encode($return_data) );		
		}
		
		$message = $_POST['message'];
		if(empty($message)){ 
			$return_data['value'] = __('Please enter message.', 'gdl_front_end');
			die ( json_encode($return_data) );				
		}
		
		$receiver = $_POST['receiver'];
		
		$messages = __("You have received a new contact form message.", 'gdl_front_end') . " \n";
		$messages = $messages . __('Name : ', 'gdl_front_end') . $name . " \n";
		$messages = $messages . __('Email : ', 'gdl_front_end') . $email . " \n";
		$messages = $messages . __('Message : ', 'gdl_front_end') . $message;
		
		$header = "From: " . $name . " <" . $email . "> \r\n";
		$header = $header . "Reply-To: " . $receiver . " \r\n";
		$header = $header . 'Content-Type: text/plain; charset=UTF-8 ' . " \r\n";
		
		if( wp_mail($receiver, __('New contact form received', 'gdl_front_end'), $messages, $header) ){
